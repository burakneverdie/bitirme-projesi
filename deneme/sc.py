...
def fetch_data_from_website(url):
    ...
    comments = soup.find_all('p') # bu kısımda web sayfasına ve çekeceğimiz veriye göre değişiklik yapmamız gerekebilir
...
def save_to_txt(comments, filename="comments.txt"):
    with open(filename, "w", encoding="utf-8") as file:
        for comment in comments:
            file.write(comment + "\n")


def load_from_txt(filename="comments.txt"):
    with open(filename, "r", encoding="utf-8") as file:
        return [line.strip() for line in file]


# girilen URL'den veri çekme
...
save_to_txt(comments_from_website)
comments_from_txt = load_from_txt()

# gelen veri üzerinde duygu analizi yapma
for comment in comments_from_txt:
