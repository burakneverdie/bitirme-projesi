...
# print(f"Test Doğruluk Oranı: {accuracy:.2f}") modelin doğruluk oranı
...
def fetch_data_from_website(url):
    ...
    #print(get.status_code) siteden dönen kodu kontrol etmek için
...
# gelen veri üzerinde duygu analizi yapma ve dosyaya kaydetme
output_file_path = "C:/Users/burakk/PycharmProjects/bitirme-projesi/outputs/deneme.txt"
with open(output_file_path, 'w', encoding='utf-8') as file:
 for comment in comments_from_website:
    ...
    output_str += f"Tahmin: {predicted_label[0]} (%{predicted_proba:.2f})\n"
    output_str += "Tüm Sınıflar İçin Oranlar:\n"
    for label, proba in class_probabilities.items():
        output_str += f"{label}: %{proba:.2f}\n"
